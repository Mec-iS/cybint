...
\item \textit{“scale”} (in terms on multilevel, multilayer, multidimensional analysis possible for a system; the scale the text aims to provide tools for is what is defined as fractal scale. Briefly here the scale at which data manifest challenges for state-of-the-art heuristics, for a general idea of this scale see [31]); also, for an intution about which kind of dimensional scale this is referring to, see mirroring in organisations and systems designed by organisations as defined in \cite{Conway1967HOWDC}.
\end{itemize}
